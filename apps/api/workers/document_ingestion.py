...
async def _run_async_ingestion(
    ...
    try:
        ...
        async with async_session_factory() as db:
            ...
            if doc_model:
                ...
                if user and user.email:
                    ...
                    trust_score = float(analysis_result.trust_score_json.get("score", 100.0) if analysis_result and analysis_result.trust_score_json else 100.0)
        ...
        logger.error(f"[Worker] Fatal error ingesting doc={document_id}: {exc}", exc_info=True)
        try:
            async with async_session_factory() as db:
                from repositories.document import DocumentRepository
                doc_repo = DocumentRepository(db)
