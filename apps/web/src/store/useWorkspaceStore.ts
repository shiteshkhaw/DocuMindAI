...
interface WorkspaceState {
  ...
  createWorkspace: (name: string, description?: string, organizationId?: string) => Promise<Workspace>;
    ...
    fetchWorkspaces: async () => {
      ...
      try {
        ...
        set({ 
          workspaces, 
          isLoading: false, 
          activeWorkspaceId: get().activeWorkspaceId || (workspaces.length > 0 ? workspaces[0].id : null) 
        });
      } catch (err) {
