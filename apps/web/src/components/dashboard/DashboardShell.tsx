...
function EntityRelationshipGraph({ entities }: { entities: any[] }) {
  const topEntities = React.useMemo(() => {
    return [...entities].sort((a, b) => (b.frequency || 1) - (a.frequency || 1)).slice(0, 12);
  }, [entities]);
    ...
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full select-none">
      ...
      <g>
        {nodes.map((node, idx) => {
          ...
          const isDimmed =
            hoveredNode && !isHovered && !node.related_entities?.includes(hoveredNode);
  ...
  messages: [
    {
      ...
      role: "user",
      content:
        "Analyze the headcount targets and check for any compliance or contractual inconsistencies between these two documents.",
      ...
      citations: [
        {
          ...
          documentName: "Sales_Headcount_Contract_2026.pdf",
          pageNumber: 8,
          snippet:
        ...
        {
          documentId: "doc-simulated-2",
          documentName: "Alpha_Project_Charter_v2.docx",
          pageNumber: 4,
          snippet:
...
const simulatedContradictions = [
  {
    ...
    conflictingStatements: [
      {
        page: 4,
        text: "Project Alpha mandates the onboarding of 60 full-time resources by Q4 2026.",
      },
  ...
  },
  {
    ...
    explanation:
      "The Project Charter sets the initial kickoff and deployment date for October 10, 2026. However, the Headcount Contract states the effective start date for vendor support personnel is November 1, 2026, creating a 22-day operational gap where resources are not contracted to perform services.",
    citations: [
      {
        documentName: "Alpha_Project_Charter_v2.docx",
        pageNumber: 1,
...
const simulatedTrustScore = {
  ...
  deductions: [
    {
      component: "numerical consistency",
      points: 8.5,
      evidence:
        "Severe mismatch in resource allocation headcount between Charter (60) and Contract (45).",
    },
    {
      component: "timeline alignment",
      points: 4.5,
      evidence: "Kickoff schedule (Oct 10) precedes contract commencement date (Nov 1).",
    },
    {
      component: "completeness",
      points: 3.0,
      evidence: "Missing explicit notice term definitions for the termination clause.",
    },
  ],
  evidence:
    "The workspace contains 2 documents with an average trust score of 84.0%. The documents are mostly compliant but contain high-risk resource conflicts and a minor timeline misalignment.",
};

const simulatedExecutiveSummary: ExecutiveSummary = {
  executive_summary:
    "This report covers the audit of Project Alpha's resources and contractual bounds. The documents contain a core operational conflict: the project plan requires 60 personnel, while the budget agreement only funds 45 vendor allocations. Kickoff dates are also out of sync with contract billing cycles.",
  key_findings: [
  ...
  important_entities: ["DocuMind Corp (Vendor)", "Acme Inc (Customer)", "Project Alpha (Scope)"],
  requirements_overview:
    "The workspace contains contradictory hiring requirements (60 vs 45 headcount) and timeline alignment discrepancies.",
  trust_assessment:
    "High probability of operational friction and cost overruns due to misaligned constraints. Budget must be revised to fund 60 resources or charter targets must scale down.",
  recommended_actions: [
...
const simulatedReviewCopilot: ReviewCopilot = {
  reviewer_checklist: [
    {
      id: "chk-1",
      category: "hiring",
      item: "Verify headcount limits align across deliverables",
      status: "fail",
      detail: "Charter states 60, contract states 45.",
      evidence: "Appendix A cap vs Sec 3.2 targets",
    },
    {
      id: "chk-2",
      category: "legal",
      item: "Ensure effective date covers kickoff date",
      status: "fail",
      detail: "Nov 1 start date leaves Oct 10-31 unbilled.",
      evidence: "Kickoff timeline alignment",
    },
    {
      id: "chk-3",
      category: "structure",
      item: "Verify signature section completeness",
      status: "pass",
      detail: "All vendor and customer sign-off blocks present.",
      evidence: "Sec 8 signed sheets",
    },
  ],
  open_questions: [
  ...
  risk_items: ["Project delay due to staffing shortage if headcount contract is not amended."],
  verification_tasks: ["Verify with finance if budget exists for 60 headcount."],
};
...
const simulatedEntityAnalysis: DocumentAnalysis = {
  ...
  summary: {
    abstract:
      "This analysis covers the structural and operational entities inside the Headcount Contract and Project Charter. Key entities including organizations (DocuMind Corp, Acme Inc), signing officers, dates, and quantitative resource targets have been extracted and mapped to analyze cross-document links.",
    keyPoints: [
  ...
  entities: [
    {
      ...
      type: "ORGANIZATION",
      frequency: 8,
      related_entities: ["Acme Inc", "John Doe"],
      confidence: 0.99,
      mentions: [{ page: 1, snippet: "DocuMind Corp agreement details" }],
    },
    {
      id: "ent-2",
      text: "Acme Inc",
      type: "ORGANIZATION",
      frequency: 6,
      related_entities: ["DocuMind Corp", "John Doe"],
      confidence: 0.99,
      mentions: [{ page: 1, snippet: "Acme Inc agreement details" }],
    },
    {
      id: "ent-3",
      text: "John Doe",
      type: "PERSON",
      frequency: 4,
      related_entities: ["DocuMind Corp", "Acme Inc"],
      confidence: 0.95,
      mentions: [{ page: 12, snippet: "Signed by John Doe" }],
    },
    {
      id: "ent-4",
      text: "October 10, 2026",
      type: "DATE",
      frequency: 3,
      related_entities: ["DocuMind Corp"],
      confidence: 0.9,
      mentions: [{ page: 1, snippet: "kickoff October 10, 2026" }],
    },
    {
      id: "ent-5",
      text: "November 1, 2026",
      type: "DATE",
      frequency: 3,
      related_entities: ["Acme Inc"],
      confidence: 0.9,
      mentions: [{ page: 2, snippet: "effective November 1, 2026" }],
    },
    {
      id: "ent-6",
      text: "60 resources",
      ...
      related_entities: ["DocuMind Corp"],
      confidence: 0.85,
      mentions: [{ page: 4, snippet: "60 full-time resources" }],
    },
    {
      id: "ent-7",
      text: "45 resources",
      ...
      related_entities: ["Acme Inc"],
      confidence: 0.85,
      mentions: [{ page: 8, snippet: "maximum of 45 resources" }],
    },
  ],
  entityConflicts: [
    {
      entity_type: "headcount",
      values: ["60 resources", "45 resources"],
      pages: [4, 8],
      description:
        "Project Charter mandates hiring 60 resources, conflicting with the Contract limit of 45 resources.",
    },
  ],
  keyValuePairs: [
    ...
    { id: "kv-3", key: "Kickoff Date", value: "October 10, 2026", confidence: 0.9 },
    { id: "kv-4", key: "Contract Billing Start", value: "November 1, 2026", confidence: 0.9 },
  ],
};
...
export default function DashboardShell() {
  ...
  const handleLaunchRecruiterTour = () => {
    ...
    addLog(
      "INFO",
      "Use the tabs above to explore Contradictions, Entities, and Compliance reports.",
    );
  };
  ...
  const [activeModel, setActiveModel] = React.useState<"documind-v3" | "deepseek-r1" | "claude-35">(
    "documind-v3",
  );
  ...
  const [wsModalTab, setWsModalTab] = React.useState<"workspaces" | "orgs" | "members" | "audit">(
    "workspaces",
  );
  ...
  const handleRateLimitError = React.useCallback(
    (err: any) => {
      ...
      if (
        ...
        setRateLimitWarning(
          "⚠️ Rate Limit Exceeded. You have made too many requests. Please wait a moment.",
        );
    ...
    <div className="flex flex-col h-full bg-sidebar-background overflow-hidden">
      ...
      <div className="p-4 border-b border-sidebar-border flex items-center justify-between shrink-0">
        <Dropdown>
          ...
          <DropdownContent
            align="left"
            className="w-56 bg-card border-card-border text-foreground shadow-lg rounded-xl p-1.5 z-50"
          >
              ...
              <DropdownItem
                ...
                icon={
                  <GitBranch
                    className={cn(
                      "h-4 w-4",
                      ws.id === activeWorkspaceId ? "text-primary" : "text-muted-foreground",
                    )}
                  />
                }
        ...
        <div className="hidden md:block">
          <Badge
            variant="outline"
            className="border-primary/25 text-primary bg-primary/5 text-[9px] font-medium leading-none py-1 px-2 rounded-full"
          >
            v1.0
          </Badge>
        </div>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive
                      ? "text-foreground font-bold animate-fade-in"
                      ...
                      <div className="overflow-hidden w-full">
                        <div className="flex items-center gap-1.5 justify-between">
                          ...
                          <Badge
                            ...
                            className={cn(
                              "text-[8px] font-medium leading-none tracking-tight scale-90 px-1.5 py-0.5 border-0 uppercase rounded-full",
                              {
                        ...
                        <div className="flex items-center gap-2 mt-1 text-[9px] text-muted-foreground">
                          ...
                          <span>•</span>
                          <span>
                            {Math.max(4, Math.floor(doc.metadata.fileSize / 8000))} chunks
                          </span>
                        </div>
                      </div>
                    </div>
  ...
  const [activeTab, setActiveTab] = React.useState<
    ...
    | "testlab"
  >("chat");
  const [selectedContradictionDocId, setSelectedContradictionDocId] = React.useState<string | null>(
    null,
  );
  ...
  const [expandedReasoningMap, setExpandedReasoningMap] = React.useState<
  ...
  const [intelligenceSubTab, setIntelligenceSubTab] = React.useState<
    "overview" | "review" | "ambiguities" | "references" | "requirements"
  >("overview");
  ...
  const [retrievalDiagnosticsMap, setRetrievalDiagnosticsMap] = React.useState<
    Record<string, RetrievalDiagnostics>
  >({});
    ...
    try {
      ...
      for await (const chunk of generator) {
        if (chunk.type === "status") {
          ...
          addLog(
            "SUCCESS",
            `Contradiction scan completed. Found ${chunk.metrics.contradictionCount} conflict(s).`,
          );
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg =
        err instanceof Error ? err.message : "Streaming failed. Check that the backend is running.";
            ...
            <div className="flex-1 flex flex-col min-h-0 relative">
                ...
                <div className="flex items-center gap-1 overflow-x-auto py-1.5 z-0">
                  {(
                    [
                      ...
                      {
                        id: "contradictions",
                        label: "Contradiction Intelligence",
                      ...
                      },
                      {
                        ...
                        icon: <Network className="h-3.5 w-3.5 text-emerald-500" />,
                      },
                      {
                        id: "diagnostics",
                        label: "Diagnostics",
              ...
              {activeTab === "chat" ? (
                <>
                  ...
                  <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-8 select-text">
                    <div className="max-w-3xl mx-auto space-y-8">
                      {activeSession.messages.map((msg) => {
                        const isUser = msg.role === "user";
                        const hasThought =
                          msg.content.includes("<thought>") && msg.content.includes("</thought>");
                        ...
                        return (
                          ...
                          >
                            ...
                            <div
                              className={cn(
                                "max-w-2xl px-5 py-4 rounded-2xl shadow-xs transition-all duration-300 leading-relaxed font-sans text-sm",
                                {
                                  "bg-primary text-primary-foreground select-text shadow-sm":
                                    isUser,
                                  "bg-card text-foreground border border-card-border select-text":
                                    !isUser,
                                },
                              )}
                            >
                                ...
                                <div className="border border-border rounded-xl bg-secondary/30 mb-3 overflow-hidden shadow-2xs">
                                  ...
                                  >
                                    <span className="flex items-center gap-2">
                                      ...
                                      Reasoning Trace (
                                      {expandedThoughtId === msg.id ? "Expanded" : "Collapsed"})
                                    </span>
                              ...
                              <div className="prose dark:prose-invert max-w-none text-sm font-sans space-y-2">
                                {msg.content === "" && isStreaming ? (
                                  <div className="flex flex-col gap-1.5 py-1">
                                    ...
                                    <span className="text-xs text-muted-foreground font-medium animate-pulse mt-0.5">
                                    ...
                                    {msg.citations && msg.citations.length > 0 ? (
                                      msg.citations.map((cit, idx) => (
                                        <Tooltip key={idx} content={cit.snippet}>
                                          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-card-border bg-secondary/50 text-[10px] text-foreground font-medium hover:bg-secondary transition-colors cursor-help">
                                            <FileText className="h-3 w-3 text-muted-foreground" />
                                            <span>
                                              {cit.documentName.substring(0, 14)}
                                              {cit.documentName.length > 14 ? "..." : ""}
                                            </span>
                                      ...
                                      <span className="text-[10px] text-muted-foreground italic">
                                        No context references matching query
                                      </span>
                                    )}
                                  </div>
                  ...
                  <div className="p-6 border-t border-border bg-background/80 backdrop-blur-md flex-shrink-0">
                    <div className="max-w-3xl mx-auto space-y-4">
                      ...
                      <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-thin">
                        ...
                        <button
                          onClick={() =>
                            handlePresetTrigger("Provide a concise summary of the active document.")
                          }
                        ...
                        <button
                          onClick={() =>
                            handlePresetTrigger(
                              "Extract key system requirements and entity interactions.",
                            )
                          }
                          className="px-3 py-1.5 text-xs font-medium border border-border bg-card text-muted-foreground hover:text-foreground hover:bg-secondary/40 rounded-full transition-all duration-200 shadow-2xs"
                        >
                        ...
                        <button
                          onClick={() =>
                            handlePresetTrigger(
                              "Verify system configurations and list missing dependencies.",
                            )
                          }
                          className="px-3 py-1.5 text-xs font-medium border border-border bg-card text-muted-foreground hover:text-foreground hover:bg-secondary/40 rounded-full transition-all duration-200 shadow-2xs"
                        >
                      ...
                      <form
                        ...
                        className="relative flex items-center bg-card border border-card-border hover:border-border transition-all duration-300 rounded-2xl shadow-sm focus-within:ring-2 focus-within:ring-primary/20 p-1.5"
                      >
                        <Input
                          type="text"
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 border border-card-border bg-card/60 rounded-2xl shadow-xs">
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Analyze vectors to detect inconsistent claims, timelines, or
                            requirements.
                          </p>
                        </div>
                        <div className="flex items-center gap-3.5">
                          ...
                          {activeSession.documentIds.length > 1 ? (
                            ...
                            <span className="text-xs font-semibold bg-secondary/35 border border-border px-3 py-2 rounded-xl">
                              {documents.find((d) => d.id === selectedContradictionDocId)?.name ||
                                "1 Target Document"}
                            </span>
                          )}
                          ...
                          <Button
                            onClick={() =>
                              ...
                              runContradictionScan(selectedContradictionDocId)
                            }
                        ...
                        <div className="flex flex-col items-center justify-center p-16 border border-dashed border-border bg-card/10 rounded-2xl text-center space-y-4">
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-muted-foreground max-w-sm">
                              Trigger a contradiction scan using the header control above to audit
                              statements, dates, and numbers.
                            </p>
                          </div>
                        </div>
                      )}
                        ...
                        <div className="space-y-4">
                          {contradictionInsights.map((insight) => {
                            ...
                            const isEvidenceOpen =
                              expandedReasoningMap[insight.id]?.evidence ?? false;
                            const isTimelineOpen =
                              expandedReasoningMap[insight.id]?.timeline ?? false;
                            ...
                            return (
                              ...
                              >
                                ...
                                <div
                                  onClick={() =>
                                    setExpandedContradictionId(isExpanded ? null : insight.id)
                                  }
                                ...
                                >
                                  <div className="space-y-2.5">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <Badge
                                        variant="outline"
                                        className="text-[8px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-full border-border bg-secondary/20 text-muted-foreground"
                                      >
                                        {insight.type}
                                      </Badge>
                                  ...
                                  <ChevronDown
                                    className={cn(
                                      "h-4.5 w-4.5 text-muted-foreground transition-transform duration-300 shrink-0 mt-1",
                                      {
                                      ...
                                      <div className="p-5 space-y-4">
                                          ...
                                          >
                                            ...
                                            <ChevronDown
                                              className={cn(
                                                ...
                                                isWhyOpen && "transform rotate-180",
                                              )}
                                            />
                                          </button>
                                          <AnimatePresence initial={false}>
                                        ...
                                        {insight.type === "timeline" && (
                                          <div className="border border-border rounded-xl bg-card overflow-hidden shadow-2xs">
                                            <button
                                              onClick={() =>
                                                toggleReasoningPanel(insight.id, "timeline")
                                              }
                                              className="w-full px-4 py-2.5 flex items-center justify-between text-xs text-foreground font-semibold hover:bg-secondary/30 transition-colors"
                                            >
                                              <span className="flex items-center gap-2">
                                              ...
                                              {isTimelineOpen && (
                                                ...
                                                >
                                                  <div className="px-4 pb-4 pt-2 text-xs text-muted-foreground leading-relaxed">
                                                    <p className="font-semibold text-foreground mb-3 font-sans">
                                        ...
                                        {insight.type === "numerical" && (
                                          <div className="border border-border rounded-xl bg-card overflow-hidden shadow-2xs">
                                            <button
                                              onClick={() =>
                                                toggleReasoningPanel(insight.id, "numerical")
                                              }
                                              className="w-full px-4 py-2.5 flex items-center justify-between text-xs text-foreground font-semibold hover:bg-secondary/30 transition-colors"
                                            >
                                              <span className="flex items-center gap-2">
                                              ...
                                              <ChevronDown
                                                className={cn(
                                                  "h-3.5 w-3.5 transition-transform",
                                                  isNumOpen && "transform rotate-180",
                                                )}
                                              />
                                            </button>
                                            <AnimatePresence initial={false}>
                                              {isNumOpen && (
                                                ...
                                                >
                                                  <div className="px-4 pb-4 pt-2 text-xs text-muted-foreground leading-relaxed">
                                                    <p className="font-semibold text-foreground mb-1.5 font-sans">
                                                      Quantitative audit:
                                                    </p>
                                                        ...
                                                        (stmt: any, idx: number) => (
                                                          <div key={idx} className="text-xs">
                                                            <span className="font-bold text-foreground font-mono">
                                                              Statement {idx + 1} (Page {stmt.page}
                                                              ):
                                                            </span>{" "}
                    ...
                    <div className="max-w-4xl mx-auto space-y-8">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 border border-card-border bg-card/60 rounded-2xl shadow-xs">
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Trust score, executive brief, review checklist, ambiguities, references
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <Button
                            onClick={async () => {
                              ...
                              addLog(
                                "INFO",
                                `Running Document Intelligence for doc: ${intelligenceDocId}`,
                              );
                              try {
                                ...
                                addLog(
                                  "SUCCESS",
                                  `Intelligence report ready. Trust score: ${typeof ts.score === "number" ? ts.score.toFixed(1) : "N/A"}`,
                                );
                              } catch (err: any) {
                      ...
                      {isLoadingIntelligence && (
                        <div className="flex flex-col items-center justify-center p-16 border border-dashed border-border bg-card/25 rounded-2xl space-y-6">
                          ...
                          <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-mono">
                            Trust · Summary · Review · Ambiguity · References · Requirements
                          </p>
                          <div className="w-64 h-1.5 bg-secondary rounded-full overflow-hidden relative">
                            <motion.div
                              initial={{ left: "-100%" }}
                        ...
                        (!trustScore || typeof trustScore.score !== "number") && (
                          <div className="flex flex-col items-center justify-center p-16 border border-dashed border-border bg-card/10 rounded-2xl text-center space-y-4">
                            ...
                            <div className="space-y-1">
                              ...
                              <p className="text-xs text-muted-foreground max-w-sm">
                                Click Run Intelligence to generate a full document trust score,
                                executive summary, review checklist, and traceability audit.
                              </p>
                            </div>
                          </div>
                        )}
                                  ...
                                  <div className="flex flex-col md:flex-row items-center gap-8">
                                    ...
                                    <div className="relative flex items-center justify-center shrink-0">
                                      <svg viewBox="0 0 120 120" className="w-36 h-36">
                                        ...
                                        <motion.circle
                                          ...
                                          animate={{
                                            strokeDasharray: `${(trustScore.score / 100) * 314} 314`,
                                          }}
                                      ...
                                      <div className="absolute text-center">
                                        <span
                                          className={cn(
                                            "text-3xl font-extrabold font-mono block",
                                            trustScore.score >= 80
                                              ? "text-emerald-500"
                                              : trustScore.score >= 60
                                                ? "text-amber-500"
                                    ...
                                    <div className="flex-1 space-y-3 w-full">
                                      ...
                                      {(trustScore.deductions || []).map((d, i) => {
                                        ...
                                        const deductionComp =
                                          d.component || d.dimension || "Deduction";
                                          ...
                                          <div key={i} className="space-y-1">
                                            ...
                                            <div className="h-1.5 bg-secondary rounded-full overflow-hidden">
                                              <motion.div
                                                initial={{ width: 0 }}
                                                animate={{
                                                  width: `${Math.min(100, deductionPts * 5)}%`,
                                                }}
                                      ...
                                      {(trustScore.deductions || []).length === 0 && (
                                        <p className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold">
                                          ✓ No significant deductions — document scores clean.
                                        </p>
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  <div className="p-6 border border-card-border bg-card rounded-2xl shadow-xs space-y-5">
                                    ...
                                    {(executiveSummary.recommended_actions || []).length > 0 && (
                                      <div className="space-y-2">
                                        ...
                                        <ul className="space-y-1.5">
                                          {(executiveSummary.recommended_actions || []).map(
                                            (a, i) => (
                                              <li
                                                key={i}
                                                className="text-xs text-foreground flex items-start gap-2"
                                    ...
                                    <ul className="space-y-1.5">
                                      {(reviewCopilot.verification_tasks || []).map((t, i) => (
                                        <li
                                          ...
                                          className="text-xs text-foreground flex items-start gap-2"
                                        >
                                          <ClipboardList className="h-3.5 w-3.5 text-sky-500 mt-0.5 shrink-0" />
                                          {t}
                                        </li>
                                      ))}
                                    </ul>
                                ...
                                {ambiguities.length === 0 ? (
                                  ...
                                  <>
                                    <div className="grid grid-cols-3 gap-3">
                                      {(["high", "medium", "low"] as const).map((sev) => {
                                        const count = ambiguities.filter(
                                          (a) => a.severity === sev,
                                        ).length;
                                        return (
                                          <div
                                            key={sev}
                                            className={cn(
                                              "p-4 rounded-xl border flex items-center gap-3 shadow-2xs",
                                              sev === "high"
                                                ? "bg-rose-500/5 border-rose-500/20 text-rose-700 dark:text-rose-400"
                                                : sev === "medium"
                                                  ? "bg-amber-500/5 border-amber-500/20 text-amber-700 dark:text-amber-400"
                                      ...
                                      {ambiguities.map((a, i) => (
                                        ...
                                        >
                                          <div className="flex items-center gap-2">
                                            <Badge
                                              className={cn(
                                                "text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border-0",
                                                a.severity === "high"
                            ...
                            {intelligenceSubTab === "references" && (
                              <div className="space-y-4">
                                {references.length === 0 ? (
                                  ...
                                  <>
                                    <div className="grid grid-cols-3 gap-3">
                                      {(["resolved", "unresolved", "external"] as const).map(
                                        (st) => {
                                    ...
                                    <div className="space-y-2">
                                      {references.map((ref, i) => (
                                        <div
                                          key={i}
                                          className="p-4 border border-card-border bg-card rounded-xl flex items-start gap-3 shadow-2xs"
                                        >
                                          <Link
                                            className={cn(
                                          ...
                                          <div className="flex-1 space-y-0.5">
                                            <div className="flex items-center gap-2">
                                              ...
                                              <Badge
                                                className={cn(
                                                  "text-[8px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded-full border-0",
                                                  ref.status === "resolved"
                                ...
                                {requirements.length === 0 ? (
                                  ...
                                  <>
                                    <div className="grid grid-cols-4 gap-3">
                                      {(
                                        ["DEFINED", "REFERENCED", "ORPHANED", "MISSING"] as const
                                      ).map((st) => {
                                        ...
                                        return (
                                          <div
                                            key={st}
                                            className={cn(
                                              "p-3 rounded-xl border text-center shadow-2xs",
                                              st === "DEFINED"
                                                ? "bg-emerald-500/5 border-emerald-500/20 text-emerald-700 dark:text-emerald-400"
                                                : st === "REFERENCED"
                                                  ? "bg-sky-500/5 border-sky-500/20 text-sky-700 dark:text-sky-400"
                    ...
                    <div className="max-w-4xl mx-auto space-y-8">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 border border-card-border bg-card/60 rounded-2xl shadow-xs">
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            LLM-powered extraction of named entities, key facts, and entity
                            conflicts from document content.
                          </p>
                        </div>
                        <div className="flex items-center gap-3">
                          {activeSession && activeSession.documentIds.length > 1 && (
                      ...
                      {!isLoadingEntities && !entityAnalysis && (
                        <div className="flex flex-col items-center justify-center p-16 border border-dashed border-border bg-card/10 rounded-2xl text-center space-y-4">
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-muted-foreground max-w-sm">
                              Click Extract Entities to run LLM-powered extraction on the selected
                              document.
                            </p>
                          </div>
                        </div>
                        ...
                        <>
                          ...
                          {entityAnalysis.summary && (
                            <div className="p-5 border border-card-border bg-card rounded-2xl space-y-3 shadow-xs">
                              ...
                              <p className="text-xs text-muted-foreground leading-relaxed">
                                {entityAnalysis.summary.abstract}
                              </p>
                          ...
                          {((entityAnalysis as any).entityConflicts || []).length > 0 && (
                            <div className="space-y-3">
                              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">
                                Entity Conflicts Detected
                              </span>
                              {((entityAnalysis as any).entityConflicts || []).map(
                                (conflict: any, i: number) => (
                                  <div
                                    key={i}
                                    className="p-4 border border-amber-500/25 bg-amber-500/5 rounded-xl space-y-2"
                                  >
                                    <div className="flex items-center gap-2">
                                      <AlertCircle className="h-4 w-4 text-amber-500" />
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {(conflict.values || []).map((v: string, vi: number) => (
                                        <span
                                          key={vi}
                                          className="text-[10px] px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-700 dark:text-amber-300 font-mono border border-amber-500/20"
                                        >
                                          {v}
                                        </span>
                                      ))}
                                    ...
                                    <p className="text-[10px] text-muted-foreground">
                                      Found on pages: {(conflict.pages || []).join(", ")}
                                    </p>
                                  </div>
                                ),
                              )}
                          ...
                          {(entityAnalysis.entities || []).length > 0 && (
                            <div className="p-5 border border-card-border bg-card rounded-2xl space-y-3 shadow-xs">
                              ...
                              <p className="text-xs text-muted-foreground">
                                Visual map of co-occurrences and semantic relationships between top
                                entities in the document text.
                              </p>
                          ...
                          {(entityAnalysis.entities || []).length > 0 && (
                            <div className="space-y-3">
                              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">
                                {(entityAnalysis.entities || []).length} entities extracted
                              </span>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {(entityAnalysis.entities || []).map((entity) => {
                                  const hasConflict = (entityAnalysis.entityConflicts || []).some(
                                    (c: any) =>
                                      (c.values || []).some(
                                        (v: string) =>
                                          v.toLowerCase().trim() ===
                                  ...
                                  return (
                                    <div
                                      key={entity.id}
                                      className="p-4 border border-card-border bg-card rounded-xl space-y-2.5 shadow-2xs hover:shadow-xs transition-all duration-200"
                                    >
                                      <div className="flex items-center justify-between">
                                        <Badge className="text-[8px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-full bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-0">
                                          {entity.type}
                                        </Badge>
                                      ...
                                      <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
                                        <span className="flex items-center gap-1">
                                          ...
                                          {entity.frequency || (entity.mentions || []).length || 1}×
                                          mentions
                                        </span>
                                          ...
                                          <div className="pt-2 border-t border-border mt-2 space-y-1">
                                            ...
                                            <div className="flex flex-wrap gap-1">
                                              {(entity.related_entities || []).map(
                                                (rel: string, ri: number) => (
                                                  <span
                                                    key={ri}
                                                    className="text-[9px] px-2 py-0.5 rounded-full bg-secondary text-muted-foreground font-medium border border-border"
                                                  >
                                                    {rel}
                                                  </span>
                                                ),
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="space-y-1.5">
                                {(entityAnalysis.keyValuePairs || []).map((kv) => (
                                  <div
                                    key={kv.id}
                                    className="flex items-center gap-3 p-3 border border-card-border bg-card rounded-xl text-xs"
                                  >
                  ...
                  <div className="flex-1 overflow-y-auto p-8 space-y-8">
                    <div className="max-w-4xl mx-auto space-y-8">
                      <div className="p-6 border border-card-border bg-card/60 rounded-2xl shadow-xs space-y-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Per-message retrieval diagnostics showing which chunks were retrieved,
                          semantic vs keyword scores, and query expansion.
                        </p>
                      </div>
                      ...
                      {Object.keys(retrievalDiagnosticsMap).length === 0 ? (
                        <div className="flex flex-col items-center justify-center p-16 border border-dashed border-border bg-card/10 rounded-2xl text-center space-y-4">
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-muted-foreground max-w-sm">
                              Send a chat message and retrieval diagnostics will appear here
                              automatically.
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="space-y-6">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {Object.entries(retrievalDiagnosticsMap).map(([msgId, diag]) => (
                              ...
                              >
                                {diag.original_query.substring(0, 30)}
                                {diag.original_query.length > 30 ? "..." : ""}
                              </button>
                            ))}
                          </div>
                          ...
                          {selectedDiagMsgId &&
                            retrievalDiagnosticsMap[selectedDiagMsgId] &&
                            (() => {
                                ...
                                <div className="space-y-5">
                                  ...
                                  <div className="p-5 border border-card-border bg-card rounded-2xl space-y-3 shadow-xs">
                                    <div className="space-y-2">
                                      <div className="flex items-start gap-2">
                                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider w-28 shrink-0 mt-0.5">
                                          Original Query
                                        </span>
                                      ...
                                      {diag.expanded_query && (
                                        <div className="flex items-start gap-2">
                                          <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider w-28 shrink-0 mt-0.5">
                                            Expanded Query
                                          </span>
                                          <span className="text-xs text-foreground font-medium">
                                            {diag.expanded_query}
                                  ...
                                  <div className="space-y-3">
                                    {diag.chunks.map((chunk, i) => (
                                      <div
                                        key={i}
                                        className="p-4 border border-card-border bg-card rounded-xl shadow-2xs space-y-3"
                                      >
                                        <div className="flex items-center justify-between">
                                          <div className="flex items-center gap-2">
                                            <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">
                                          ...
                                          <div className="flex items-center gap-3 text-[10px]">
                                            ...
                                            <div className="flex-1 h-1.5 bg-secondary rounded-full overflow-hidden">
                                              <motion.div
                                                initial={{ width: 0 }}
                                                animate={{
                                                  width: `${chunk.semantic_score * 100}%`,
                                                }}
                                                transition={{ duration: 0.8, ease: "easeOut" }}
                                            ...
                                            <span className="text-muted-foreground font-mono w-10 text-right">
                                              {(chunk.semantic_score * 100).toFixed(0)}%
                                            </span>
                                          </div>
                                          <div className="flex items-center gap-3 text-[10px]">
                                            <span className="text-muted-foreground w-24">
                                            ...
                                            <span className="text-muted-foreground font-mono w-10 text-right">
                                              {(chunk.keyword_score * 100).toFixed(0)}%
                                            </span>
                                          </div>
                                        </div>
                                      </div>
                    ...
                    <div className="max-w-4xl mx-auto space-y-8">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 border border-card-border bg-card/60 rounded-2xl shadow-xs">
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Live status of all infrastructure components powering DocuMind AI.
                          </p>
                        </div>
                        <Button
                          onClick={async () => {
                      ...
                      {!healthStatus && !isLoadingHealth && (
                        <div className="flex flex-col items-center justify-center p-16 border border-dashed border-border bg-card/10 rounded-2xl text-center space-y-4">
                          ...
                          <p className="text-xs text-muted-foreground max-w-sm">
                            Click Run Health Check to verify all infrastructure components.
                          </p>
                        </div>
                      )}
                        ...
                        <div className="space-y-6">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {[
                              {
                                label: "PostgreSQL",
                                value: healthStatus.postgres,
                                icon: <Database className="h-4 w-4 text-indigo-500" />,
                              },
                              {
                                label: "Chroma Vector DB",
                                value: `${healthStatus.chroma} (${healthStatus.chroma_backend})`,
                                icon: <Brain className="h-4 w-4 text-primary" />,
                              },
                              {
                                label: "Embedding Provider",
                                value: healthStatus.embedding_provider,
                                icon: <Cpu className="h-4 w-4 text-amber-500" />,
                              },
                              {
                                label: "Collections",
                                value: `${healthStatus.chroma_collection_count} collection(s)`,
                                icon: <BarChart2 className="h-4 w-4 text-emerald-500" />,
                              },
                            ].map(({ label, value, icon }) => (
                              <div
                                key={label}
                                className="p-4 border border-card-border bg-card rounded-xl shadow-2xs flex items-start gap-3"
                              >
                          ...
                          <div className="p-5 border border-card-border bg-card rounded-2xl space-y-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {healthStatus.llm_providers.map((p) => (
                                <span
                                  key={p}
                                  className="text-xs px-3 py-1 rounded-full border border-card-border bg-secondary/50 text-foreground font-medium"
                                >
                                  {p}
                                </span>
                              ))}
                    ...
                    <div className="max-w-4xl mx-auto space-y-8">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 border border-card-border bg-card/60 rounded-2xl shadow-xs">
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-muted-foreground text-left">
                            Run a comprehensive system-level validation harness checking functional
                            metrics, security controls, and tenant isolation.
                          </p>
                        </div>
                        <Button
                          onClick={async () => {
                            ...
                            setSuiteLogs([
                              "[Suite] Booting test harness...",
                              "[Suite] Provisioning sandbox workspaces...",
                            ]);
                            try {
                              ...
                              addLog(
                                "SUCCESS",
                                `Validation suite completed: ${res.summary.status}`,
                              );
                            } catch (err: any) {
                              setSuiteLogs((prev) => [
                                ...prev,
                                `[ERROR] Validation suite crashed: ${err.message}`,
                              ]);
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 border border-card-border bg-card/60 rounded-2xl shadow-xs">
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-muted-foreground text-left">
                            Load a pre-configured simulation workspace with dual-source documents,
                            cross-document conflicts, entity telemetry, and RAG logs for debugging
                        ...
                        <div className="space-y-8 animate-fade-in text-left">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="p-4 border border-card-border bg-card rounded-xl shadow-2xs">
                              ...
                              <span
                                className={cn("text-lg font-extrabold mt-1 block", {
                            ...
                            <div className="p-4 border border-card-border bg-card rounded-xl shadow-2xs">
                              ...
                              <span className="text-lg font-bold text-foreground mt-1 block font-sans">
                                ...
                                {validationReport.summary.total_tests}
                              </span>
                            </div>
                            <div className="p-4 border border-card-border bg-card rounded-xl shadow-2xs">
                              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {Object.entries(validationReport.test_packs).map(
                                ([level, pack]: [string, any]) => (
                                  <div
                                    key={level}
                                    className="p-5 border border-card-border bg-card rounded-2xl shadow-2xs space-y-4"
                                  >
                                    <div className="flex items-center justify-between">
                                      ...
                                      <Badge
                                        variant={pack.status === "PASS" ? "success" : "error"}
                                    ...
                                    <div className="space-y-2 text-xs border-b border-border/40 pb-3">
                                      <div className="flex justify-between text-muted-foreground">
                                        ...
                                        <span className="font-bold text-foreground font-sans">
                                          {pack.metrics.contradictions_found}
                                        </span>
                                      </div>
                                      <div className="flex justify-between text-muted-foreground">
                                        <span>Requirements Traced:</span>
                                        <span className="font-bold text-foreground font-sans">
                                          {pack.metrics.requirements_traced.length}
                                        </span>
                                      </div>
                                      <div className="flex justify-between text-muted-foreground">
                                        <span>Trust Score:</span>
                                        <span className="font-bold text-foreground font-sans">
                                          {pack.metrics.trust_score.toFixed(1)}%
                                        </span>
                                      </div>
                                      {pack.metrics.entities_extracted.length > 0 && (
                                        <div className="text-muted-foreground mt-1">
                                          <span>Entities: </span>
                                          <span className="font-semibold text-foreground text-[10px] bg-secondary/80 py-0.5 px-1.5 rounded">
                                            {pack.metrics.entities_extracted.slice(0, 3).join(", ")}
                                          </span>
                                        </div>
                                      )}
                                    </div>
                        ...
                        <div className="bg-black/5 dark:bg-black/20 font-mono text-[10px] p-4 rounded-xl max-h-48 overflow-y-auto space-y-1 scrollbar-thin select-all border border-border/30">
                          {suiteLogs.length === 0 ? (
                            <span className="text-muted-foreground">
                              Test suite idle. Click &quot;Run Validation Suite&quot; to begin.
                            </span>
                          ) : (
                              ...
                              <div
                                ...
                                className={cn("whitespace-pre-wrap", {
                                  ...
                                  "text-emerald-500 font-semibold":
                                    log.includes("Result: PASS") || log.includes("SUCCESS"),
                                  "text-muted-foreground":
                                    !log.startsWith("[ERROR]") && !log.includes("Result: PASS"),
                                })}
                              >
                                {log}
                              </div>
                ...
                <div className="relative border border-card-border bg-card rounded-2xl p-8 overflow-hidden shadow-xs">
                  ...
                  <div className="relative space-y-4 z-10">
                    ...
                    <p className="text-sm text-muted-foreground max-w-2xl leading-relaxed">
                      Analyze, extract, and converse with multiple document datasets. DocuMind
                      indexes text vectors to provide secure, cross-source RAG insights and
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <Card className="bg-card border-card-border rounded-2xl shadow-2xs hover:shadow-xs transition-all duration-200">
                      <CardContent className="p-5 flex items-center gap-4">
                        ...
                        <div>
                          <span className="text-xs text-muted-foreground font-medium block leading-none">
                            Indexed sources
                          </span>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div className="p-5 border border-card-border bg-card rounded-2xl space-y-3 hover:shadow-2xs transition-all duration-200">
                      ...
                      <p className="text-xs text-muted-foreground leading-relaxed">
                        Drag and drop your PDF, Word, or txt documents in the sidebar to extract
                        semantic chunks and build vector embeddings.
                      </p>
                    </div>
                    ...
                    <div className="p-5 border border-card-border bg-card rounded-2xl space-y-3 hover:shadow-2xs transition-all duration-200">
                      ...
                      <p className="text-xs text-muted-foreground leading-relaxed">
                        Select one or more documents from your active knowledge base. The AI will
                        target its contextual analysis to these active documents.
                      </p>
                    </div>
                    ...
                    <div className="p-5 border border-card-border bg-card rounded-2xl space-y-3 hover:shadow-2xs transition-all duration-200">
                      ...
                      <p className="text-xs text-muted-foreground leading-relaxed">
                        Click compile to initialize a conversational context. Execute summaries or
                        pose direct queries with precise source mapping.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="p-5 border border-card-border bg-card rounded-2xl font-sans text-xs space-y-3.5 text-muted-foreground shadow-2xs">
                    <div className="flex items-center justify-between text-muted-foreground">
                      <span className="font-mono select-all p-1 bg-secondary/50 rounded border border-border/30">
                        $ npm i @documind/sdk
                      </span>
                      <button
                    ...
                    <div className="border-t border-border pt-3.5 flex items-center justify-between">
                      <span className="font-mono select-all p-1 bg-secondary/50 rounded border border-border/30">
                        const sdk = new DocuMindSDK(&#123; activeModel: &quot;{activeModel}&quot;
                        &#125;)
                      </span>
                      <button
                        onClick={() =>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <ChevronDown
                  className={cn("h-4 w-4 text-muted-foreground transition-transform duration-300", {
              ...
              <div className="flex-1 overflow-y-auto p-3.5 space-y-1.5 bg-black/5 dark:bg-black/20 font-mono text-[10px] leading-relaxed">
                {systemLogs.map((log) => (
                  <div key={log.id} className="flex gap-2.5 items-start">
                    ...
                    <span
                      className={cn(
                        "font-bold shrink-0 uppercase tracking-tight select-none text-[9px] px-1 rounded",
                        {
                    ...
                    <span
                      className={cn("text-foreground whitespace-pre-wrap select-all", {
          ...
          <ModalContent className="max-w-2xl bg-card border-card-border shadow-xl rounded-2xl p-6 text-foreground">
            <ModalHeader>
              ...
              <ModalDescription className="text-xs text-muted-foreground">
                Manage workspaces, user organizations, access control roles, and review workspace
                audit history.
              </ModalDescription>
            </ModalHeader>
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              {isActive && (
                                <Badge
                                  variant="outline"
                                  className="text-[9px] bg-primary/10 text-primary border-primary/20 rounded-full scale-90 px-1.5 py-0.5"
                                >
                                  Active
                                </Badge>
                              )}
                      ...
                      <form
                        onSubmit={async (e) => {
                          ...
                          try {
                            ...
                            await sdk.addOrganizationMember(
                              selectedOrgId,
                              inviteUserId.trim(),
                              inviteRole,
                            );
              ...
              {wsModalTab === "audit" && (
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-3 bg-secondary/20 rounded-xl border border-border/50">
                    ...
                    <Badge
                      variant="outline"
                      className="text-[9px] bg-secondary text-muted-foreground border-border rounded-full py-0.5 px-2 font-mono"
                    >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="max-h-[260px] overflow-y-auto space-y-2 pr-1 scrollbar-thin">
                      {!auditSelectedWorkspaceId ? (
                        <p className="text-xs text-muted-foreground p-4 border border-dashed rounded-xl text-center">
                          Select a workspace to view its audit logs.
                        </p>
                      ) : workspaceAuditLogs.length === 0 ? (
                        <p className="text-xs text-muted-foreground p-4 border border-dashed rounded-xl text-center">
                          No action logs found for this workspace. Actions like document upload or
